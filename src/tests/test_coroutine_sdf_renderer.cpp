...
int main(int argc, char *argv[]) {
    ...
    Coroutine render_coro = [&](Var<CoroFrame> &frame, ImageUInt seed_image, ImageFloat accum_image, UInt frame_index) noexcept {
        ...
        Float ux = rand(seed);
        Float uy = rand(seed);
        Float2 uv = make_float2(coord.x + ux, resolution.y - 1u - resolution.y + uy);
    ...
    auto frame_buffer = device.create_buffer<CoroFrame>(width * height);

    LUISA_INFO("Recorded AST in {} ms.", clock.toc());

    Kernel2D render_kernel_entry = [&](ImageUInt seed_image, ImageFloat accum_image, UInt frame_index) noexcept {
        ...
        render_coro(frame, seed_image, accum_image, frame_index);
        frame_buffer->write(coord_1d, frame);
    };
    auto render_entry = device.compile(render_kernel_entry);

    Kernel2D render_kernel_resume = [&](ImageUInt seed_image, ImageFloat accum_image, UInt frame_index) noexcept {
        set_block_size(16u, 8u, 1u);
        ...
        $switch (token) {
            for (int i = 1; i <= render_coro.suspend_count(); ++i) {
                $case (i) {
                    render_coro[i](frame, seed_image, accum_image, frame_index);
                };
            }
            $default {
                // TODO
    ...
    auto render_resume = device.compile(render_kernel_resume);
    ...
    for (uint spp = 0u; spp < total_spp; spp += interval) {
        ...
        for (uint frame = spp; frame < spp + interval && frame < total_spp; frame++) {
            command_list << render_entry(seed_image, accum_image, frame).dispatch(width, height);

            for (auto i = 0u; i < max_ray_depth * render_coro.suspend_count() * 2u; i++)
                command_list << render_resume(seed_image, accum_image, frame).dispatch(width, height);
