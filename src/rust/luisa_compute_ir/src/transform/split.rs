...
use crate::ir::{
    new_node, BasicBlock, CallableModule, CallableModuleRef, Capture, Func, Instruction, IrBuilder,
    KernelModule, Module, ModulePools, Node, NodeRef, PhiIncoming, StructType, SwitchCase, Type,
    INVALID_REF,
};
        ...
        while node_ref_present != visit_state.end {
            ...
            match instruction {
                ...
                Instruction::CoroSplitMark { token } => {
                    ...
                    let (sb_before, sb_after) =
                        self.visit_coro_split_mark(scope_builder, *token, node_ref_present.clone());
                    let visit_state_after = VisitState::new(
                        visit_state.get_bb_ref(),
                        Some(node_ref_present.get().next),
                        None,
                        None,
                    );
                ...
                Instruction::Loop { body, cond } => {
                    ...
                    let mut visit_result =
                        self.visit_loop(pools, scope_builder, visit_state_after, body, cond);
    ...
    fn duplicate_node(&mut self, scope_builder: &mut ScopeBuilder, node_ref: NodeRef) -> NodeRef {
        ...
        let dup_node = match instruction {
            ...
            Instruction::If {
                ...
                let dup_cond = self.find_duplicated_node(frame_token, *cond);
                let dup_true_branch =
                    self.duplicate_block(frame_token, &builder.pools, true_branch);
                let dup_false_branch =
                    self.duplicate_block(frame_token, &builder.pools, false_branch);
            ...
            Instruction::Switch {
                ...
                let dup_cases: Vec<_> = cases
                    ...
                    .map(|case| {
                        let dup_block =
                            self.duplicate_block(frame_token, &builder.pools, &case.block);
                        SwitchCase {
                            value: case.value,
                            block: dup_block,
                        }
            ...
            Instruction::RayQuery {
                ...
                let dup_on_triangle_hit =
                    self.duplicate_block(frame_token, &builder.pools, on_triangle_hit);
                let dup_on_procedural_hit =
                    self.duplicate_block(frame_token, &builder.pools, on_procedural_hit);
                builder.ray_query(
                    dup_ray_query,
                    dup_on_triangle_hit,
