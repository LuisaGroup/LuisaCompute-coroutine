...
int main(int argc, char *argv[]) {
    ...
    static constexpr auto max_depth = 10u;
    ...
    Coroutine raytracing_coro = [&](Var<CoroFrame> &frame, ImageFloat image, ImageUInt seed_image, AccelVar accel, UInt2 resolution) noexcept {
        auto coord = coro_id().xy();
    ...
    LUISA_INFO_WITH_LOCATION("Coro count = {}", raytracing_coro.suspend_count());
    auto type = Type::of<CoroFrame>();
    auto frame_buffer = device.create_buffer<CoroFrame>(resolution.x * resolution.y);

    Kernel2D entry_kernel = [&](ImageFloat image, ImageUInt seed_image, AccelVar accel, UInt2 resolution) noexcept {
    ...
    Kernel2D resume_kernel = [&](ImageFloat image, ImageUInt seed_image, AccelVar accel, UInt2 resolution) noexcept {
        auto coord = dispatch_id().xy();
        auto coord_1d = coord.y * resolution.y + coord.x;
        auto frame = frame_buffer->read(coord_1d);
        auto token = read_promise<uint>(frame, "coro_token");
        $switch (token) {
            for (int i = 1; i <= raytracing_coro.suspend_count(); ++i) {
                $case (i) {
    ...
    auto ray_tracing_entry_shader = device.compile(entry_kernel);
    auto ray_tracing_resume_shader = device.compile(resume_kernel);
    ...
    while (!window.should_close()) {
        cmd_list << ray_tracing_entry_shader(framebuffer, seed_image, accel, resolution)
                        .dispatch(resolution);
        for (auto iter = 0u; iter < spp_per_dispatch * raytracing_coro.suspend_count() * 2u; ++iter) {
            cmd_list << ray_tracing_resume_shader(framebuffer, seed_image, accel, resolution)
                            .dispatch(resolution);
        }
