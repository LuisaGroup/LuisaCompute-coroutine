...
impl CoroFrameAnalyser {
    ...
    fn visit_coro_split_mark(
        ...
        if visited {
            ...
            // record continuation
            self.continuations
                .entry(fb_before.token)
                .or_insert(Continuation::new(fb_before.token))
                .next
                .insert(token_next);
            self.continuations
                .entry(token_next)
    ...
    fn visit_if(
        ...
        let fb_false =
            self.visit_branch_split(frame_builder.token, false_branch, &mut fb_after_vec);
        assert_eq!(
            fb_false.finished,
            self.split_possibility
